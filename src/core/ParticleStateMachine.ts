...
export interface StateContext {
    nearbyParticles: Map<number, Particle[]>; // Grouped by species
    localDensity: number;
...
export class ParticleStateMachine {
    private state: ParticleState = ParticleState.EXPLORING;
    private stateTimer: number = 0;
    private previousState: ParticleState = ParticleState.EXPLORING;
    private target?: Particle;
    private behaviorMemory: { x: number; y: number }[] = [];
    private maxMemoryLength: number = 30;
    
    private readonly behaviorParams: BehaviorParams = {
        exploreSpeed: 1.0,
        huntSpeed: 2.0,
        fleeSpeed: 2.5,
        clusterDistance: 50,
    ...
    constructor(initialState: ParticleState = ParticleState.EXPLORING) {
        this.state = initialState;
    }
    
    public update(particle: Particle, context: StateContext, dt: number): void {
        this.stateTimer += dt;
    ...
    private transitionTo(newState: ParticleState): void {
    ...
    private executeBehavior(particle: Particle, context: StateContext, dt: number): void {
        switch (this.state) {
    ...
    private exploreMovement(particle: Particle, dt: number): void {
    ...
    private huntMovement(particle: Particle, context: StateContext, dt: number): void {
        if (!context.nearestPrey) return;
    ...
    private fleeMovement(particle: Particle, context: StateContext, dt: number): void {
        if (!context.nearestPredator) return;
    ...
    private clusterMovement(particle: Particle, context: StateContext, dt: number): void {
    ...
    private matingMovement(particle: Particle, context: StateContext, dt: number): void {
        if (!context.nearestMate) return;
    ...
    private foragingMovement(particle: Particle, context: StateContext, dt: number): void {
        // Follow energy gradient
        const gradientStrength = Math.sqrt(
            context.energyGradient.x * context.energyGradient.x + 
    ...
    private dormantBehavior(particle: Particle, dt: number): void {
    ...
    private dyingBehavior(particle: Particle, dt: number): void {
    ...
    private updateMemory(particle: Particle): void {
    ...
    public getTarget(): Particle | undefined {
        return this.target;
    }
    
    public getBehaviorParams(): BehaviorParams {
        return this.behaviorParams;
    }
    
    public getMemoryTrail(): { x: number; y: number }[] {
